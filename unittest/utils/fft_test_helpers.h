...
//
// Internal Design Docs: FFT_TESTER_PHASE2_SUMMARY.md (section 2: Validation Strategy Design)
//                       FFT_TESTER_RESEARCH.md (section 1.6 for FFT_DATA structure)

#ifndef FFT_TEST_HELPERS_H
#define FFT_TEST_HELPERS_H
...
inline FFT_SCALAR grid_scale_factor(int nfast, int nmid, int nslow)
{
    ...
    return std::sqrt(static_cast<FFT_SCALAR>(n_total) / 1000.0);
}
...
inline std::complex<FFT_SCALAR> get_complex(const FFT_SCALAR *data, int i, int j, int k,
                                            int nfast, int nmid)
{
...
// =============================================================================
// SUBSECTION 1.3: Configuration Reporting
// =============================================================================

// Get FFT configuration string (library, precision, backend info)
inline std::string get_fft_config_string()
{
    std::string config = "FFT Library: ";
    config += LMP_FFT_LIB;
    config += ", Precision: ";
    config += LMP_FFT_PREC;

#if defined(FFT_FFTW_THREADS) || defined(FFT_MKL_THREADS)
    config += ", Threading: enabled";
#endif

#if defined(FFT_KOKKOS_CUFFT)
    config += ", Backend: CUDA (cuFFT)";
#elif defined(FFT_KOKKOS_HIPFFT)
    config += ", Backend: HIP (hipFFT)";
#elif defined(FFT_KOKKOS_MKL_GPU)
    config += ", Backend: SYCL (MKL GPU)";
#endif
...
public:
    ...
    std::string description() const override { return "Delta function at origin"; }
...
public:
    ...
    std::string description() const override { return "Constant field"; }
...
public:
        ...
        for (int k = 0; k < nslow; k++) {
            for (int j = 0; j < nmid; j++) {
                for (int i = 0; i < nfast; i++) {
                    ...
                    double phase = TWO_PI *
                        (kx_ * static_cast<double>(i) / nfast +
                    ...
                    set_complex(data, i, j, k, nfast, nmid,
                                std::complex<FFT_SCALAR>(value, 0.0));
                }
            }
        }
    }
    ...
    {
        return "Sine wave (kx=" + std::to_string(kx_) + ", ky=" + std::to_string(ky_) +
            ", kz=" + std::to_string(kz_) + ")";
    }
...
public:
    ...
    std::string description() const override
    {
        return "3D Gaussian (sigma=" + std::to_string(sigma_) + ")";
    }
...
public:
    ...
    std::string description() const override
    {
        return "Random complex (seed=" + std::to_string(seed_) + ")";
    }
...
public:
    ...
    void generate(FFT_SCALAR *data, int nfast, int nmid, int nslow) override
    {
        ...
        for (const auto &mode : modes_) {
            for (int k = 0; k < nslow; k++) {
                for (int j = 0; j < nmid; j++) {
                    for (int i = 0; i < nfast; i++) {
                        double wave_phase = TWO_PI *
                            (mode.kx * static_cast<double>(i) / nfast +
    ...
    std::string description() const override
    {
        return "Mixed modes (" + std::to_string(modes_.size()) + " frequencies)";
    }
...
public:
    RoundTripValidator(const FFT_SCALAR *original_data, const FFT_SCALAR *recovered_data,
                       int nfast, int nmid, int nslow, FFT_SCALAR tolerance = ROUNDTRIP_TOLERANCE,
                       bool verbose = false)
    ...
    std::string description() const override { return "Round-trip (IFFT(FFT(x)) = x)"; }
...
public:
    KnownAnswerValidator(const FFT_SCALAR *computed_fft, const FFT_SCALAR *expected_fft,
                         int nfast, int nmid, int nslow,
                         FFT_SCALAR tolerance = KNOWN_ANSWER_TOLERANCE, bool verbose = false)
        : Validator(verbose), computed_fft_(computed_fft), expected_fft_(expected_fft),
          nfast_(nfast), nmid_(nmid), nslow_(nslow), tolerance_(tolerance)
    {
    }
    ...
    bool validate() override
    {
        ...
        if (verbose_) {
            std::cout << "Known answer validation: " << (passed ? "PASSED" : "FAILED")
                      << std::endl;
            std::cout << "Error stats: " << error_stats_ << std::endl;
        }
    ...
    std::string description() const override { return "Known answer comparison"; }
...
public:
    ParsevalValidator(const FFT_SCALAR *spatial_data, const FFT_SCALAR *frequency_data,
                      int nfast, int nmid, int nslow, FFT_SCALAR tolerance = PARSEVAL_TOLERANCE,
                      bool verbose = false)
    ...
    bool validate() override
    {
        ...
        // Relative error
        double error = (expected_frequency_energy > 1e-14) ?
            abs_error / expected_frequency_energy :
            abs_error;
    ...
    std::string description() const override { return "Parseval's theorem (energy conservation)"; }
...
public:
    ...
    std::string description() const override
    {
        return "Hermitian symmetry (X(k) = X*(-k) for real input)";
    }
...
public:
    LinearityValidator(const FFT_SCALAR *fft_sum,         // FFT(a·x + b·y)
                       ...
                       int nfast, int nmid, int nslow,
                       FFT_SCALAR tolerance = LINEARITY_TOLERANCE, bool verbose = false)
    ...
    std::string description() const override
    {
        return "Linearity (FFT(a·x+b·y) = a·FFT(x)+b·FFT(y))";
    }
 ...
 *
 * // Create test data
 * int nfast = 64, nmid = 64, nslow = 64;
 * FFTBuffer original(nfast, nmid, nslow);
 * FFTBuffer fft_result(nfast, nmid, nslow);
 * FFTBuffer recovered(nfast, nmid, nslow);
 *
 * // Generate Gaussian test data
 * GaussianGenerator generator(2.0);
 * generator.generate(original.data(), nfast, nmid, nslow);
 *
 * // Perform forward + backward FFT
 * FFT3d *fft = new FFT3d(lmp, comm, nfast, nmid, nslow, ...);
 * fft->compute(original.data(), fft_result.data(), FFT3d::FORWARD);
 * fft->compute(fft_result.data(), recovered.data(), FFT3d::BACKWARD);
 *
 * // Validate round-trip
 * RoundTripValidator validator(original.data(), recovered.data(),
 *                              nfast, nmid, nslow,
 *                              scaled_tolerance(ROUNDTRIP_TOLERANCE, nfast, nmid, nslow),
 *                              true);
 ...
 *
 * ---------------------------------
 * EXAMPLE 2: Parseval's Theorem Test
 * ---------------------------------
 *
 * // Create data and compute FFT
 * FFTBuffer spatial(nfast, nmid, nslow);
 * FFTBuffer frequency(nfast, nmid, nslow);
 *
 * SineWaveGenerator generator(1, 0, 0);  // Single frequency along x-axis
 * generator.generate(spatial.data(), nfast, nmid, nslow);
 *
 * fft->compute(spatial.data(), frequency.data(), FFT3d::FORWARD);
 *
 * // Validate energy conservation
 * ParsevalValidator validator(spatial.data(), frequency.data(),
 *                             nfast, nmid, nslow,
 *                             scaled_tolerance(PARSEVAL_TOLERANCE, nfast, nmid, nslow),
 *                             true);
 * EXPECT_TRUE(validator.validate());
 *
 * ---------------------------------
 * EXAMPLE 3: Hermitian Symmetry Test
 * ---------------------------------
 *
 * // Generate real-valued input
 * FFTBuffer real_input(nfast, nmid, nslow);
 * FFTBuffer fft_output(nfast, nmid, nslow);
 *
 * ConstantGenerator generator(1.0);
 * generator.generate(real_input.data(), nfast, nmid, nslow);
 *
 * fft->compute(real_input.data(), fft_output.data(), FFT3d::FORWARD);
 *
 * // Validate Hermitian symmetry
 * HermitianSymmetryValidator validator(fft_output.data(), nfast, nmid, nslow,
 *                                     scaled_tolerance(HERMITIAN_TOLERANCE, nfast, nmid, nslow),
 *                                     true);
 * EXPECT_TRUE(validator.validate());
 *
 * ---------------------------------
 ...
 * FFTBuffer x(nfast, nmid, nslow), y(nfast, nmid, nslow);
 * SineWaveGenerator gen_x(1, 0, 0);
 * SineWaveGenerator gen_y(0, 1, 0);
 * gen_x.generate(x.data(), nfast, nmid, nslow);
 * gen_y.generate(y.data(), nfast, nmid, nslow);
 *
 * // Compute FFT(x), FFT(y)
 * FFTBuffer fft_x(nfast, nmid, nslow), fft_y(nfast, nmid, nslow);
 * fft->compute(x.data(), fft_x.data(), FFT3d::FORWARD);
 * fft->compute(y.data(), fft_y.data(), FFT3d::FORWARD);
 *
 * // Create linear combination: sum = a·x + b·y
 * FFT_SCALAR a = 2.0, b = 3.0;
 * FFTBuffer sum(nfast, nmid, nslow);
 * for (int i = 0; i < nfast * nmid * nslow; i++) {
 *     auto val_x = get_complex_linear(x.data(), i);
 *     auto val_y = get_complex_linear(y.data(), i);
 *     set_complex_linear(sum.data(), i, a * val_x + b * val_y);
 * }
 *
 * // Compute FFT(a·x + b·y)
 * FFTBuffer fft_sum(nfast, nmid, nslow);
 * fft->compute(sum.data(), fft_sum.data(), FFT3d::FORWARD);
 *
 * // Validate linearity
 * LinearityValidator validator(fft_sum.data(), fft_x.data(), fft_y.data(),
 *                             a, b, nfast, nmid, nslow,
 *                             scaled_tolerance(LINEARITY_TOLERANCE, nfast, nmid, nslow),
 *                             true);
 * EXPECT_TRUE(validator.validate());
 */

#endif    // FFT_TEST_HELPERS_H
