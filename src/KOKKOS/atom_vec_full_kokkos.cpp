...
void AtomVecFullKokkos::unpack_border_kokkos(const int &n, const int &first,
  ...
  if (space==Host) {
    struct AtomVecFullKokkos_UnpackBorder<LMPHostType>
      f(buf.h_view,h_x,h_tag,h_type,h_mask,h_q,h_molecule,first);
    Kokkos::parallel_for(n,f);
  } else {
    struct AtomVecFullKokkos_UnpackBorder<LMPDeviceType>
      f(buf.d_view,d_x,d_tag,d_type,d_mask,d_q,d_molecule,first);
    Kokkos::parallel_for(n,f);
  }
...
/* ---------------------------------------------------------------------- */
int AtomVecFullKokkos::unpack_exchange_kokkos(DAT::tdual_double_2d_lr &k_buf, int nrecv, int nlocal,
...
void AtomVecFullKokkos::sync(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
    ...
    if (mask & DIHEDRAL_MASK) {
      atomKK->k_num_dihedral.sync_host();
      atomKK->k_dihedral_type.sync_host_kk();
      atomKK->k_dihedral_atom1.sync_host_kk();
      atomKK->k_dihedral_atom2.sync_host_kk();
      atomKK->k_dihedral_atom3.sync_host_kk();
      atomKK->k_dihedral_atom4.sync_host_kk();
    }
    if (mask & IMPROPER_MASK) {
      atomKK->k_num_improper.sync_host();
      atomKK->k_improper_type.sync_host_kk();
      atomKK->k_improper_atom1.sync_host_kk();
      atomKK->k_improper_atom2.sync_host_kk();
      atomKK->k_improper_atom3.sync_host_kk();
      atomKK->k_improper_atom4.sync_host_kk();
    }
  }
}
  ...
  if (space == Device) {
    ...
    if (mask & ANGLE_MASK) {
      ...
      if (atomKK->k_angle_type.need_sync_device())
        perform_async_copy<DAT::tdual_int_2d>(atomKK->k_angle_type.k_view,space);
      if (atomKK->k_angle_atom1.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_angle_atom1.k_view,space);
      if (atomKK->k_angle_atom2.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_angle_atom2.k_view,space);
      if (atomKK->k_angle_atom3.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_angle_atom3.k_view,space);
    }
    if (mask & DIHEDRAL_MASK) {
      ...
      if (atomKK->k_dihedral_type.need_sync_device())
        perform_async_copy<DAT::tdual_int_2d>(atomKK->k_dihedral_type.k_view,space);
      if (atomKK->k_dihedral_atom1.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_dihedral_atom1.k_view,space);
      if (atomKK->k_dihedral_atom2.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_dihedral_atom2.k_view,space);
      if (atomKK->k_dihedral_atom3.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_dihedral_atom3.k_view,space);
    }
    if (mask & IMPROPER_MASK) {
      ...
      if (atomKK->k_improper_type.need_sync_device())
        perform_async_copy<DAT::tdual_int_2d>(atomKK->k_improper_type.k_view,space);
      if (atomKK->k_improper_atom1.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_improper_atom1.k_view,space);
      if (atomKK->k_improper_atom2.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_improper_atom2.k_view,space);
      if (atomKK->k_improper_atom3.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_improper_atom3.k_view,space);
      if (atomKK->k_improper_atom4.need_sync_device())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_improper_atom4.k_view,space);
    }
  } else {
    ...
    if (mask & DIHEDRAL_MASK) {
      ...
      if (atomKK->k_dihedral_atom4.need_sync_host())
        perform_async_copy<DAT::tdual_tagint_2d>(atomKK->k_dihedral_atom4.k_view,space);
    }
    if (mask & IMPROPER_MASK) {
...
void AtomVecFullKokkos::modified(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
    ...
    if (mask & DIHEDRAL_MASK) {
      atomKK->k_num_dihedral.modify_host();
      atomKK->k_dihedral_type.modify_host_kk();
      atomKK->k_dihedral_atom1.modify_host_kk();
      atomKK->k_dihedral_atom2.modify_host_kk();
      atomKK->k_dihedral_atom3.modify_host_kk();
      atomKK->k_dihedral_atom4.modify_host_kk();
    }
    if (mask & IMPROPER_MASK) {
      atomKK->k_num_improper.modify_host();
      atomKK->k_improper_type.modify_host_kk();
      atomKK->k_improper_atom1.modify_host_kk();
      atomKK->k_improper_atom2.modify_host_kk();
      atomKK->k_improper_atom3.modify_host_kk();
      atomKK->k_improper_atom4.modify_host_kk();
    }
  }
}
