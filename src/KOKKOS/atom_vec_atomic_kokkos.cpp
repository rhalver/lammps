...
void AtomVecAtomicKokkos::unpack_border_kokkos(const int &n, const int &first,
  ...
  if (space==Host) {
    struct AtomVecAtomicKokkos_UnpackBorder<LMPHostType> f(buf.view_host(),h_x,h_tag,h_type,h_mask,first);
    Kokkos::parallel_for(n,f);
  } else {
    struct AtomVecAtomicKokkos_UnpackBorder<LMPDeviceType> f(buf.view_device(),d_x,d_tag,d_type,d_mask,first);
    Kokkos::parallel_for(n,f);
  }
...
void AtomVecAtomicKokkos::sync_pinned(ExecutionSpace space, uint64_t mask, int async_flag)
{
  if (space == Device) {
...
void AtomVecAtomicKokkos::modified(ExecutionSpace space, uint64_t mask)
{
  if (space == Device) {
